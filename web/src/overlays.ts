...
export function open_overlay(opts: OverlayOptions): void {
    ...
    if (active_overlay?.$element || open_overlay_name || active_overlay?.close_handler) {
        blueslip.error(
...
export function close_overlay(name: string): void {
    ...
    const {$element: $active_overlay, close_handler} = active_overlay;
...
export function close_modal(selector: string, conf: Pick<ModalConfig, "on_hidden"> = {}): void {
    if (selector === undefined) {
    ...
    if (active_modal() !== `#${selector}`) {
        blueslip.error(
            `Trying to close ${selector} modal when ${active_modal() || "undefined"} is open.`,
        );
        return;
    }
